...
!            Federal University of Rio de Janeiro, Brazil
  ...
  subroutine tPlaymol_write_internals( me, unit, three ) !converts a N;#comment;name xyz;name xyz; name xyz to a N;#comment;name; name name bond; name name bond name angle. --- the opposite of build via bonds and angles, working only for a trimer. for developer testing i guess, idk...
    class(tPlaymol),  intent(inout) :: me
    integer,          intent(in)    :: unit
    character(sl),    intent(in)    :: three(3) !three arguments corresponding to unique atom names
    ...
    if (abs( abs(cos_angle(atom_index(three))) - 1.0_rb ) < 1.0e-2_rb) then
      call error( "atoms", join(three), "are almost colinear: choose other ones" )
    end if
    rev3 = three(3:1:-1) !reverse order the input array
    i3 = atom_index(rev3)
    !xyz based format
    write(unit,'(A)') trim(int2str(N)) !first line = number of atoms
    write(unit,'("# Generated by Playmol on ",A)') trim(now()) !comment line
    ...
    contains
      ...
      subroutine search( list, iatom, found, D )
        ...
        do while (.not.found .and. associated(ptr))
          D = ptr%id
          if (ptr % match_id(C)) found = count(done(atom_index(D))) == m-1
          ptr => ptr % next
        end do
        if (found .and. (D(m) == iatom)) D = D(m:1:-1)
      end subroutine search
      !---------------------------------------------------------------------------------------------
      elemental function atom_index( iatom ) result( i )
        character(sl), intent(in) :: iatom
        integer                   :: i
        i = 1
